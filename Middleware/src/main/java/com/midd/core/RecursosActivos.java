...
public class RecursosActivos {
	...
	public ResponseEntity<?> agregarActivo(@RequestBody Activos activo){
		try {
			...
			if (serviciosActivos.validarMAC(activo.getDireccion_mac())==false) {
			...
			if (serviciosActivos.validarIP(activo.getDireccion_ip())==false) {
			...
			if (serviciosActivos.validarFecha(activo.getFecha_entrega(), activo.getFecha_registro())) {
				logger.warn("La fecha de entrega "+activo.getFecha_entrega()+" no puede ser mayor a la fecha de registro "+activo.getFecha_registro());
				return new ResponseEntity<>(respuestas.respuestas("La fecha de entrega no puede ser mayor a la fecha de registro","2004"),HttpStatus.BAD_REQUEST);
			}
	...
	public ResponseEntity<?> eliminarActivo(@RequestBody Activos activo){
		try {
			...
			logger.warn("El activo "+activo.getId_activo()+" no puede ser eliminado por el usuario "+activo.getId_ultimatix()+"!");
			return new ResponseEntity<>(respuestas.respuestas("Usuario no puede eliminar activo","2022"),HttpStatus.BAD_REQUEST);
		...
		try {
			...
			if (activo1.getId_ultimatix().equals(activo.getId_ultimatix())) {
				...
				if (serviciosActivos.validarFecha(activo1.getFecha_registro(),activo1.getFecha_devolucion())) {
					logger.warn("La fecha de devolucion "+activo1.getFecha_devolucion()+" no puede ser menor a la fecha de registro "+activo1.getFecha_registro());
					return new ResponseEntity<>(respuestas.respuestas("La fecha de devolucion no puede ser menor a la fecha de registro","2013"),HttpStatus.BAD_REQUEST);
				}
				serviciosActivos.actualizarActivo(activo1);
			...
			logger.warn("El activo "+activo.getId_activo()+" no puede ser eliminado por el usuario "+activo.getId_ultimatix()+"!");
			return new ResponseEntity<>(respuestas.respuestas("Usuario no puede devolver activo","2012"),HttpStatus.BAD_REQUEST);
	...
	public ResponseEntity<?> agregarEdificio(@RequestBody Edificio nuevo){
		...
		for (Edificio iterante : edificios) {
			if (iterante.getNombre().equals(nuevo.getNombre())) {
				return new ResponseEntity<>(respuestas.respuestas("Edificio ya registrado","400"),HttpStatus.BAD_REQUEST);
			}
		}
